...
run_many_powers <- !(max_base_powers == 1) # Flag for analysing several Dr powers at once

# Checks
if(max_base_powers > npreds) stop("max_base_powers cannot be larger than npreds")
if(max_base_powers > 1 && !run_many_powers) stop("Flag set to run only 1 power, but max_base_powers > 1")
if(max_base_powers == 1 && run_many_powers) stop("Flag set to run many powers, but max_base_powers = 1")
      ...
      for (i in 1:num_sizes) {
        ...
        for (b in 0:n_iter) {
          ...
          for (base_vars in 1:max_base_powers) { # sweep through different dr predictive powers
            if (run_many_powers) base_pred <- oracle_pred(data_hold,
                                                        coefs_base[base_vars, ], num_vars = base_vars)
            else base_pred <- oracle_pred(data_hold, coefs_base[base_vars, ])
